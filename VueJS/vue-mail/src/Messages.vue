<template>
  <table v-if="messages.length > 0" class="table table-inbox table-hover">
    <tbody>
      <tr v-for="(message, index) in messages" @click="openMessage(message)" :key="'message' + index" :class="{unread: typeof message.isRead !== 'undefined' && !message.isRead}">
  ...
  <p v-else>Not message not yet.</p>
</template>
<script>
